...
- [ ] GPU passthrough with Podman: generate CDI spec `sudo nvidia-ctk cdi generate --output=/etc/cdi/nvidia.yaml`; if `--gpus all` still fails, switch runArg to `--device nvidia.com/gpu=all`
...
- [x] A6. Regenerate template `devcontainer.json` via `./configure_devcontainer.sh --cuda --base ubuntu-24.04 --non-interactive`; verify `customizations`/`DISPLAY` preserved, CUDA 12.9, `--gpus all` present
...
- [x] `.devcontainer/devcontainer.json`: add template's VS Code extensions + `containerEnv.DISPLAY` (no ROS vars), then regenerate via configure script; confirm round-trip keeps `--gpus all`, full `remoteEnv` incl. `OPTIX_HOME`, CUDA 12.9
...
- [x] Generator round-trip (both repos): run configure twice → identical output; `--no-cuda` removes CUDA feature/`--gpus`/CUDA env but preserves `OPTIX_HOME`/`customizations`/`DISPLAY`; `--cuda` restores
